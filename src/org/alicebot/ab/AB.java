...
public class AB {
  ...
  public void productivity(int runCompletedCnt, Timer timer) {
    ...
    System.out.println(
        "Completed " + runCompletedCnt + " in " + time + " min. Productivity " + (float) runCompletedCnt / time + " cat/min");
  }
  ...
  public void skipCategory(Category c) {
    /*
     * c.setFilename(MagicStrings.unfinished_aiml_file); c.setTemplate(MagicStrings.unfinished_template);
     * bot.unfinishedGraph.addCategory(c); System.out.println(bot.unfinishedGraph.getCategories().size() +
  ...
  void findPatterns(Nodemapper node, String partialPatternThatTopic) {
    if (NodemapperOperator.isLeaf(node)) {
      ...
      if (node.category.getActivationCnt() > MagicNumbers.node_activation_cnt) {
        // System.out.println("LEAF: "+node.category.getActivationCnt()+". "+partialPatternThatTopic+" "+node.shortCut); //Start
    ...
    if (NodemapperOperator.size(node) > MagicNumbers.node_size) {
      ...
      starPatternCnt++;
      try {
        Category c = new Category(0, partialPatternThatTopic + " * <THAT> * <TOPIC> *", MagicStrings.blank_template,
  ...
  /**
   * magically suggests new patterns for a bot. Reads an input file of sample data called logFile. Builds a graph of all the
   * inputs. Finds new patterns in the graph that are not already in the bot. Classifies input log into those new patterns.
   *
   *
   *
   */
  ...
  public void terminalInteractionStep(Bot bot, String request, String textLine, Category c, String alicetemplate) {
    ...
    if (textLine.equals("q"))
      ...
      bot.writeQuit();
      /*
       * Nodemapper udcNode = bot.brain.findNode("*", "*", "*"); if (udcNode != null) { AIMLSet udcMatches = new
       * AIMLSet("udcmatches"); udcMatches.addAll(udcNode.category.getMatches()); udcMatches.writeAIMLSet(); }
       */
      /*
       * Nodemapper cNode = bot.brain.match("JOE MAKES BEER", "unknown", "unknown"); if (cNode != null) { AIMLSet cMatches = new
       * AIMLSet("cmatches"); cMatches.addAll(cNode.category.getMatches()); cMatches.writeAIMLSet(); } if (passed.size() > 0) {
       * AIMLSet difference = new AIMLSet("difference"); AIMLSet intersection = new AIMLSet("intersection"); for (String s :
       * passed) if (testSet.contains(s)) intersection.add(s); passed = intersection; passed.setName = "passed";
       * difference.addAll(testSet); difference.removeAll(passed); difference.writeAIMLSet();
       * 
       * passed.writeAIMLSet(); testSet.writeAIMLSet(); System.out.println("Wrote passed test cases"); }
       */
      System.exit(0);
