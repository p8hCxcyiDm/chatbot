...
public class Graphmaster {
  ...
  public void addCategory(Category category) {
    ...
    /*
     * if (inputThatTopic.contains("<B")) { Matcher matcher = botPropPattern.matcher(inputThatTopic); while (matcher.find()) {
     * String propname = matcher.group(1).toLowerCase(); //System.out.println(matcher.group(1)); String property =
  ...
  /**
   * add a Path to the graph from a given node. Shortcuts: Replace all instances of paths "<THAT> * <TOPIC> *" with a direct link
  ...
  void addPath(Nodemapper node, Path path, Category category) {
    // if (path != null) System.out.println("Enable shortcuts = "+enableShortCuts+" path="+Path.pathToSentence(path)+"
  ...
  Nodemapper findNode(Nodemapper node, Path path) {
    if (path == null && node != null) {
      ...
      return node;
        ...
        && Objects.requireNonNull(path).word.equals("<THAT>")) {
      if (verbose)
        System.out.println("findNode: shortcut, returning " + node.category.inputThatTopic());
      return node;
    } else if (NodemapperOperator.containsKey(node, Objects.requireNonNull(path).word)) {
      if (verbose)
  ...
  /**
   * Depth-first search of the graph for a matching leaf node. At each node, the order of search is 1. $WORD (high priority exact
   * word match) 2. # wildcard (zero or more word match) 3. _ wildcard (one or more words match) 4. WORD (exact word match) 5.
   * {@code <set></set>} (AIML Set match) 6. shortcut (graph shortcut when that pattern = * and topic pattern = *) 7. ^ wildcard
  ...
  final Nodemapper match(Path path, Nodemapper node, String inputThatTopic, String starState, int starIndex, String[] inputStars,
    ...
    if ((matchedNode = nullMatch(path, node, matchTrace)) != null)
      ...
      return null;
        ...
        dollarMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null)
      return matchedNode;
    else if ((matchedNode =
        sharpMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null)
      return matchedNode;
    else if ((matchedNode =
        underMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null)
      return matchedNode;
    else if ((matchedNode =
        wordMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null)
      return matchedNode;
    else if ((matchedNode =
        setMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null)
      return matchedNode;
    else if ((matchedNode =
        shortCutMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null)
      return matchedNode;
    else if ((matchedNode =
        caretMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null)
      return matchedNode;
    else if ((matchedNode =
        starMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null)
      return matchedNode;
    else {
      return null;
    }
  ...
  final Nodemapper wordMatch(Path path, Nodemapper node, String inputThatTopic, String starState, int starIndex,
    ...
    try {
      ...
      if (path != null && NodemapperOperator.containsKey(node, uword)
          && (matchedNode = match(path.next, NodemapperOperator.get(node, uword), inputThatTopic, starState, starIndex,
  ...
  final Nodemapper dollarMatch(Path path, Nodemapper node, String inputThatTopic, String starState, int starIndex,
    ...
    Nodemapper matchedNode;
    if (path != null && NodemapperOperator.containsKey(node, uword)
        && (matchedNode = match(path.next, NodemapperOperator.get(node, uword), inputThatTopic, starState, starIndex, inputStars,
  ...
  final Nodemapper zeroMatch(Path path, Nodemapper node, String input, String starState, int starIndex, String[] inputStars,
      String[] thatStars, String[] topicStars, String wildcard, String matchTrace) {
    ...
    if (path != null && NodemapperOperator.containsKey(node, wildcard)) {
      // System.out.println("Zero match calling setStars Prop "+MagicStrings.null_star+" =
  ...
  final Nodemapper wildMatch(Path path, Nodemapper node, String input, String starState, int starIndex, String[] inputStars,
      String[] thatStars, String[] topicStars, String wildcard, String matchTrace) {
    Nodemapper matchedNode;
    ...
    try {
      if (path != null && NodemapperOperator.containsKey(node, wildcard)) {
        ...
        if (NodemapperOperator.isLeaf(nextNode) && !nextNode.shortCut) {
          ...
          for (path = pathStart; path != null && !currentWord.equals("<THAT>") && !currentWord.equals("<TOPIC>"); path =
              path.next) {
            matchTrace += "[" + wildcard + "," + path.word + "]";
            if ((matchedNode =
                match(path, nextNode, input, starState, starIndex + 1, inputStars, thatStars, topicStars, matchTrace)) != null) {
              setStars(starWords, starIndex, starState, inputStars, thatStars, topicStars);
              return matchedNode;
            } else {
              currentWord = path.word;
  ...
  final Nodemapper setMatch(Path path, Nodemapper node, String input, String starState, int starIndex, String[] inputStars,
      String[] thatStars, String[] topicStars, String matchTrace) {
    if (DEBUG)
      System.out.println("Graphmaster.setMatch(path: " + path + ", node: " + node + ", input: " + input + ", starState: "
          + starState + ", starIndex: " + starIndex + ", inputStars, thatStars, topicStars, matchTrace: " + matchTrace + ", )");
    ...
    for (String setName : node.sets) {
      ...
      for (Path qath = path.next; qath != null && !currentWord.equals("<THAT>") && !currentWord.equals("<TOPIC>")
        ...
        if (aimlSet.contains(phrase) && (matchedNode =
            match(qath, nextNode, input, starState, starIndex + 1, inputStars, thatStars, topicStars, matchTrace)) != null) {
          setStars(starWords, starIndex, starState, inputStars, thatStars, topicStars);
          if (DEBUG)
  ...
  /*
   * final Nodemapper setMatch(Path path, Nodemapper node, String input, String starState, int starIndex, String[] inputStars,
   * String[] thatStars, String[] topicStars, String matchTrace) { if (DEBUG) System.out.println("Graphmaster.setMatch(path: " +
   * path + ", node: " + node + ", input: " + input + ", starState: " + starState + ", starIndex: " + starIndex +
   ...
   * System.out.println("in Graphmaster.setMatch, setMatch sets ="+node.sets); for (String setName : node.sets) { if (DEBUG)
   * System.out.println("in Graphmaster.setMatch, setMatch trying type "+setName); Nodemapper nextNode =
   * NodemapperOperator.get(node, "<SET>"+setName.toUpperCase()+"</SET>"); AIMLSet aimlSet = bot.setMap.get(setName);
   * //System.out.println(aimlSet.setName + "="+ aimlSet); Nodemapper matchedNode; String currentWord = path.word; String
   * starWords = currentWord+" "; int length = 1; matchTrace += "[<set>"+setName+"</set>,"+path.word+"]"; if (DEBUG)
   * System.out.println("in Graphmaster.setMatch, setMatch starWords =\""+starWords+"\""); for (Path qath = path.next; qath !=
   * null && !currentWord.equals("<THAT>") && !currentWord.equals("<TOPIC>") && length <= aimlSet.maxLength; qath = qath.next) {
   * if (DEBUG) System.out.println("in Graphmaster.setMatch, qath.word = "+qath.word); String phrase =
   * bot.preProcessor.normalize(starWords.trim()).toUpperCase(); if (DEBUG)
   * System.out.println("in Graphmaster.setMatch, setMatch trying \""+phrase+"\" in "+setName); if (aimlSet.contains(phrase) &&
   * (matchedNode = match(qath, nextNode, input, starState, starIndex + 1, inputStars, thatStars, topicStars, matchTrace)) !=
   * null) { setStars(starWords, starIndex, starState, inputStars, thatStars, topicStars); if (DEBUG)
   * System.out.println("in Graphmaster.setMatch, setMatch found "+phrase+" in "+ setName); return matchedNode; } // else if
   * (qath.word.equals("<THAT>") || qath.word.equals("<TOPIC>")) return null; else { length = length + 1; currentWord = qath.word;
   * starWords += currentWord + " "; } } } fail("set", matchTrace); return null; }
   */
  ...
  void printgraph(Nodemapper node, String partial) {
    if (node == null)
      ...
      if (NodemapperOperator.isLeaf(node) || node.shortCut) {
        ...
        if (node.shortCut)
          System.out.println(partial + "(" + NodemapperOperator.size(node) + "[" + node.height
  ...
  public void nodeStats() {
    ...
    naryCnt = 0;
    nodeStatsGraph(root);
    resultNote = bot.name + " (" + name + "): " + getCategories().size() + " categories " + nodeCnt + " nodes " + singletonCnt
