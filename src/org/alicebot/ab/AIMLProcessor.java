...
public class AIMLProcessor {
  ...
  public static String respond(String input, String that, String topic, Chat chatSession, int srCnt) {
    MagicBooleans.trace(
        "input: " + input + ", that: " + that + ", topic: " + topic + ", chatSession: " + chatSession + ", srCnt: " + srCnt);
    String response;
  ...
  public static String evalTagContent(Node node, ParseState ps, Set<String> ignoreAttributes) {
    // MagicBooleans.trace("AIMLProcessor.evalTagContent(node: " + node + ", ps: " + ps + ", ignoreAttributes: " +
    // ignoreAttributes);
  ...
  /**
   * return a string of unevaluated XML. When the AIML parser encounters an unrecognized XML tag, it simply passes through the tag
   * in XML form. For example, if the response contains HTML markup, the HTML is passed to the requesting process. However if that
   * markup contains AIML tags, those tags are evaluated and the parser builds the result.
   *
   * @param node
   *          current parse node.
   * @param ps
  ...
  private static String srai(Node node, ParseState ps) {
    ...
    try {
      ...
      if (MagicBooleans.trace_mode) {
        System.out.println(
            trace_count + ". <srai>" + result + "</srai> from " + ps.leaf.category.inputThatTopic() + " topic=" + topic + ") ");
        trace_count++;
      }
      ...
      response =
          evalTemplate(leaf.category.getTemplate(), new ParseState(ps.depth + 1, ps.chatSession, ps.input, ps.that, topic, leaf));
      // System.out.println("That="+that);
    } catch (Exception ex) {
      ex.printStackTrace();
    }
  ...
  /**
   * in AIML 2.0, an attribute value can be specified by either an XML attribute value or a subtag of the same name. This function
   * tries to read the value from the XML attribute first, then tries to look for the subtag.
   *
   * @param node
   *          current parse node.
   * @param ps
  ...
  /**
   * implements {@code <thatstar index="N"/>} returns the value of input words matching the Nth wildcard (or AIML Set) in
   * <that></that>.
   *
   * @param node
   *          current XML parse node
  ...
  /**
   * implements <topicstar/> and <topicstar index="N"/> returns the value of input words matching the Nth wildcard (or AIML Set)
   * in a topic pattern.
   *
   * @param node
   *          current XML parse node
  ...
  /**
   * implements {@code <system>} tag. Evaluate the contents, and try to execute the result as a command in the underlying OS
   * shell. Read back and return the result of this command.
   *
  ...
  /**
   * Transform a string of words (separtaed by spaces) into a string of individual characters (separated by spaces). Explode "ABC
   * DEF" = "A B C D E F".
   *
   * @param node
   *          current XML parse node
  ...
  /**
   * implements {@code <condition> with <loop/>} re-evaluate the conditional statement until the response does not contain
   * {@code <loop/>}
   *
   * @param node
   *          current XML parse node
    ...
    // otherwise this is a <condition> with <li> items:
      ...
      for (int i = 0; i < liList.size() && result.equals(""); i++) {
        ...
        if (value != null) {
          ...
          else if (liVarName != null && value != null
              && (ps.vars.get(liVarName).equalsIgnoreCase(value) || (ps.vars.containsKey(liPredicate) && value.equals("*"))))
            return evalTagContent(n, ps, attributeNames);
  ...
  /*
   * private static String qnotq(Node node, ParseState ps, Boolean affirm) { String subject = getAttributeOrTagValue(node, ps,
   * "subj"); String predicate = getAttributeOrTagValue(node, ps, "pred"); String object = getAttributeOrTagValue(node, ps,
   * "obj"); TripleStore ts = ps.chatSession.tripleStore; HashSet<String> triples = ts.getTriples(subject, predicate, object); if
   * (!affirm) { HashSet<String> U = new HashSet<String>(ts.allTriples()); U.removeAll(triples); triples = U; } return
   * ts.formatAIMLTripleList(triples); } private static String q(Node node, ParseState ps) { return qnotq(node, ps, true); }
   * private static String notq(Node node, ParseState ps) { return qnotq(node, ps, false); }
   */
